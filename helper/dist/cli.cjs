...
// src/types.ts
var SkillLevelSchema = external_exports.enum(["beginner", "intermediate", "expert"]);
var QuestionTypeSchema = external_exports.enum(["A", "B", "C", "D"]);
var VerdictSchema = external_exports.enum(["correct", "partial", "wrong"]);
var AnswerEntrySchema = external_exports.object({
...
var SessionEntrySchema = external_exports.object({
  ...
  endedAt: external_exports.string(),
  questionCount: external_exports.number().int().nonnegative(),
  correctCount: external_exports.number().int().nonnegative()
});
var StateSchema = external_exports.object({
  schemaVersion: external_exports.literal(1),
  skillLevel: SkillLevelSchema,
  lastQuizSha: external_exports.string().nullable(),
  rollingWindow: external_exports.array(AnswerEntrySchema).max(20),
  sessionHistory: external_exports.array(SessionEntrySchema)
});
...
var GlobalSymbolSchema = external_exports.object({
  name: external_exports.string(),
  file: external_exports.string(),
  kind: external_exports.enum(["function", "class", "const", "type", "other"]),
  summary: external_exports.string()
});
var MapSchema = external_exports.object({
  schemaVersion: external_exports.literal(1),
  builtAtSha: external_exports.string(),
  ...
  globalSymbols: external_exports.array(GlobalSymbolSchema),
  architectureNotes: external_exports.string()
});
var CURRENT_STATE_SCHEMA_VERSION = 1;
...
function saveState(path, state2) {
  StateSchema.parse(state2);
  (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(path), { recursive: true });
  const tmp = `${path}.tmp`;
  (0, import_node_fs.writeFileSync)(tmp, JSON.stringify(state2, null, 2), "utf8");
  (0, import_node_fs.renameSync)(tmp, path);
}
var ROLLING_WINDOW_SIZE = 20;
function recordAnswer(state2, entry) {
  const next = [...state2.rollingWindow, entry];
  const trimmed = next.length > ROLLING_WINDOW_SIZE ? next.slice(next.length - ROLLING_WINDOW_SIZE) : next;
  return { ...state2, rollingWindow: trimmed };
}
var MIN_WINDOW_FOR_DRIFT = 10;
var PROMOTE_ACCURACY = 0.8;
var DEMOTE_WRONG_FRAC = 0.6;
var HARD_TYPE_MIN = 5;
var HARD_TYPES_FOR_LEVEL = {
  beginner: ["C", "D"],
  intermediate: ["D"],
  expert: []
};
...
function applySkillDrift(state2) {
  const w = state2.rollingWindow;
  if (w.length < MIN_WINDOW_FOR_DRIFT) return state2;
...
var WEAK_MODULE_WRONG_THRESHOLD = 2;
function summarizeSession(state2, since) {
  const entries = since ? state2.rollingWindow.filter((e) => e.timestamp >= since) : state2.rollingWindow;
  ...
  const moduleWrongCounts = /* @__PURE__ */ new Map();
  ...
  const weakModules = [...moduleWrongCounts.entries()].filter(([, n]) => n >= WEAK_MODULE_WRONG_THRESHOLD).sort((a, b) => b[1] - a[1]).map(([m]) => m);
  return {
    ...
    byType,
    weakModules,
    skillLevel: state2.skillLevel
  };
}
function backupCorrupt(path) {
  const dir = (0, import_node_path.dirname)(path);
  const base = (0, import_node_path.basename)(path, ".json");
  const stamp = Date.now();
  const backup = (0, import_node_path.join)(dir, `${base}.bak.${stamp}.json`);
  try {
...
function gradeShowMe(input) {
  const u = input.userInput.trim();
  const expectedPathNorm = normalizePath(input.expectedPath);
  const expectedBase = basename2(expectedPathNorm);
  const candidates = u.split(/[|\s,;]+/).map((t) => t.replace(/[.,;:)]+$/, "")).filter(Boolean);
  let bestMatch = null;
  ...
  if (input.expectedLine !== void 0) {
    if (bestMatch.line === void 0) {
      verdict = "partial";
  ...
  if (input.expectedSnippet) {
    const withoutPaths = u.split(/\s+/).filter((t) => !/\//.test(t) && !/\.[a-zA-Z0-9]+$/.test(t.replace(/[.,;:)]+$/, ""))).join(" ");
...
// src/map.ts
var DRIFT_FILE_THRESHOLD = 20;
var MAX_AGE_DAYS = 30;
function shouldRefreshMap(input) {
...
function resolveScope(input) {
  if (input.focus !== void 0) {
    ...
    const matches = input.map.modules.filter(
      (m) => m.path.toLowerCase().includes(needle) || m.summary.toLowerCase().includes(needle)
    );
    return { modules: matches, source: "focus" };
  }
  if (input.recentChangedFiles.length > 0) {
    const matches = input.map.modules.filter(
      (m) => input.recentChangedFiles.some((f) => f.startsWith(m.path + "/") || f === m.path)
    );
...
function git(cwd, args) {
  return (0, import_node_child_process.execFileSync)("git", ["-C", cwd, ...args], { encoding: "utf8" }).trim();
}
...
function changedFilesSince(cwd, sinceSha) {
  if (sinceSha === null) {
  ...
  return Array.from(/* @__PURE__ */ new Set([...committed, ...unstaged, ...untracked]));
}

// src/cli.ts
var program2 = new Command();
program2.name("quiz-helper").description("Deterministic helpers for quiz-master plugin");
var state = program2.command("state");
state.command("init").requiredOption("--path <path>").action((opts) => {
  saveState(opts.path, defaultState());
});
state.command("get").alias("load").requiredOption("--path <path>").action((opts) => {
  process.stdout.write(JSON.stringify(loadState(opts.path)));
});
state.command("record-answer").requiredOption("--path <path>").requiredOption("--type <type>").requiredOption("--module <module>").requiredOption("--verdict <verdict>").action((opts) => {
  const entry = {
    type: opts.type,
    module: opts.module,
...
state.command("set-level").requiredOption("--path <path>").requiredOption("--level <level>").action((opts) => {
  const s = loadState(opts.path);
  saveState(opts.path, { ...s, skillLevel: opts.level });
});
state.command("set-last-sha").requiredOption("--path <path>").requiredOption("--sha <sha>").action((opts) => {
  const s = loadState(opts.path);
  saveState(opts.path, { ...s, lastQuizSha: opts.sha });
});
state.command("session-summary").requiredOption("--path <path>").option("--since <iso>").action((opts) => {
  const s = loadState(opts.path);
  process.stdout.write(JSON.stringify(summarizeSession(s, opts.since)));
});
var grade = program2.command("grade");
grade.command("mcq").requiredOption("--user-input <input>").requiredOption("--correct-letter <letter>").option("--type-your-own-letter <letter>").action((opts) => {
...
grade.command("show-me").requiredOption("--user-input <input>").requiredOption("--expected-path <path>").option("--expected-line <line>", "", (v) => parseInt(v, 10)).option("--expected-snippet <snippet>").action((opts) => {
...
map.command("check-staleness").option("--map-path <path>", "convenience: read map file for its own sha/version/age").requiredOption("--expected-schema-version <n>", "", (v) => parseInt(v, 10)).requiredOption("--head-sha <sha>").requiredOption("--changed-file-count <n>", "", (v) => parseInt(v, 10)).option("--map-schema-version <n>", "", (v) => parseInt(v, 10)).option("--map-sha <sha>").option("--map-age-days <n>", "", (v) => parseInt(v, 10)).option("--force").action((opts) => {
  ...
  if (opts.mapPath) {
    if (!(0, import_node_fs2.existsSync)(opts.mapPath)) {
      process.stdout.write(JSON.stringify({ refresh: true, reason: "missing" }));
      return;
    }
    try {
      const raw = (0, import_node_fs2.readFileSync)(opts.mapPath, "utf8");
      const parsed = MapSchema.parse(JSON.parse(raw));
      mapSchemaVersion = parsed.schemaVersion;
      mapSha = parsed.builtAtSha;
      const ms = (0, import_node_fs2.statSync)(opts.mapPath).mtimeMs;
      mapAgeDays = (Date.now() - ms) / (1e3 * 60 * 60 * 24);
    } catch {
...
map.command("save").requiredOption("--path <path>").action((opts) => {
  const raw = (0, import_node_fs2.readFileSync)(0, "utf8");
  const parsed = MapSchema.parse(JSON.parse(raw));
  (0, import_node_fs2.writeFileSync)(opts.path, JSON.stringify(parsed, null, 2));
});
map.command("resolve-scope").requiredOption("--map-path <path>").requiredOption("--state-path <path>").option("--focus <focus>").option("--recent <files>", "comma-separated").action((opts) => {
  const parsedMap = MapSchema.parse(JSON.parse((0, import_node_fs2.readFileSync)(opts.mapPath, "utf8")));
...
var git2 = program2.command("git");
git2.command("head-sha").requiredOption("--cwd <cwd>").action((opts) => {
...
git2.command("changed-since").requiredOption("--cwd <cwd>").option("--since <sha>").action((opts) => {
  if (!isGitRepo(opts.cwd)) {
